...
export interface UserRecord {
  ...
  is_active: boolean;
  created_at: string; // Assuming string format from API
  updated_at: string; // Assuming string format from API
...
export function UserDetailsSheet({ selectedUser, setSelectedUser, onUserUpdate }: UserDetailsSheetProps) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast({ title: '错误', description: (error as Error).message || '更新用户信息时发生错误', variant: 'destructive' });
    }
      ...
      <SheetContent className="sm:max-w-[525px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="username-edit" value={username} onChange={(e) => setUsername(e.target.value)} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="password-edit" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="留空则不修改" className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Switch id="status-edit" checked={isActive} onCheckedChange={setIsActive} className="col-span-3" />
          </div>
        </div>
        <SheetFooter>
          <SheetClose asChild>
